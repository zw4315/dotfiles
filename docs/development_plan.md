...
目标：把 repo 演进为“平台层 + 软件层 + 个人层”的可组合结构，同时保持安装方式简单、可回滚。
...
- **简化维护**：以 `init.sh + profiles + modules` 为唯一入口，不再维护 legacy `setup/files`。
...
dotfiles/
  ...
  scripts/
    init.sh                   # 入口：探测平台 + 选择 profile + 执行 modules
    setup_linux.sh
    setup_windows.ps1
...
- 对插件目录等大目录（例如 `~/.vim/plugged` 或 `nvim` 的插件缓存）应做 **排除或保留策略**（当前由 `modules/vim.sh` 实现保留 plugged）。
...
### Phase 0：梳理现状（1 次性）
- 盘点当前 `home/`/`config/` 里每个文件的落地点与归属（common/os/app）。
- 明确你是否主要使用：`Ubuntu`、`WSL2`、`Windows 原生`、`macOS`（决定平台优先级）。

### Phase 1：引入新布局但不破坏旧安装（小步）
- （已删除 legacy）不再提供 `setup` 入口。
- 添加 `scripts/link_tree.sh`：支持把一个目录树链接到目标根目录（含备份、dry-run、排除规则）。
- 先把 **nvim** 迁移到 `config/nvim`（已完成）。

### Phase 2：移除 legacy `files/`/`setup`
- 已完成：安装入口统一为 `init.sh`，配置源分为 `home/` + `config/`。

建议默认走 **方案 B**（长期维护最简单），但 Phase 1 可以先按 A 过渡。

### Phase 3：平台化（Linux + Windows）
- Linux：把 apt 安装逻辑收敛为模块内实现（已完成，见 `lib/apt_helpers.sh` + 各模块）。
...
- `init.sh` 在 Linux 上一键完成：安装依赖（可选）、链接配置、输出清单、可重复运行不破坏已有文件。
